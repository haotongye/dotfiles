...
-- Open/Close tabs
--[[ utils.map("n", "<leader>E", ":tabnew<SPACE>", { silent = false })
utils.map("n", "<leader>t", ":tabnew<CR>", { silent = false }) ]]

-- Navigate tabs
--[[ utils.map("n", "<Left>", "gT")
utils.map("n", "<Right>", "gt")
utils.map("i", "<Left>", "<ESC>gT")
utils.map("i", "<Right>", "<ESC>gt") ]]
...
-- Split window
utils.map("n", "<Leader>h", ":vsplit<CR>")
utils.map("n", "<Leader>v", ":split<CR>")
